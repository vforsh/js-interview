...
function moveDown(heap: number[], index: number) {
	let newIndex = getSmallestChildIndex(heap, index)
	while (newIndex < heap.length && heap[index] > heap[newIndex]) {
		swap(heap, index, newIndex)
		index = newIndex
		newIndex = getSmallestChildIndex(heap, newIndex)
	}
}
...
function getSmallestChildIndex(heap: number[], index: number): number {
